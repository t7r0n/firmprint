...
COMPANY = 'Zarna'
REPO = 'firmprint'
PROJECT_TERMS = ['zarna', 'value', 'proposition', 'associates', 'tuned', 'evaluation', 'methods', 'decision']
PROJECT_METRICS = ['zarna_coverage', 'value_risk', 'proposition_precision', 'associates_latency']
PROJECT_FAILURES = ['zarna_drift', 'value_gap', 'proposition_misroute', 'associates_blindspot']
PROJECT_ARCHETYPES = [{'name': 'zarna evidence replay', 'trigger': 'zarna signal changes while value context is stale', 'expected': 'block release until cited evidence is regenerated'}, {'name': 'value boundary probe', 'trigger': 'value handoff crosses a policy or trust boundary', 'expected': 'route to reviewer with evidence packet'}, {'name': 'proposition regression harness', 'trigger': 'proposition behavior regresses against the last accepted fixture', 'expected': 'open a regression issue with trace and benchmark delta'}, {'name': 'associates operator packet', 'trigger': 'associates output needs a human-readable audit packet', 'expected': 'accept only if decision claims cite fixture evidence'}]
PROJECT_DIRECTION = 'An open source tool that ingests a fund\'s historical memo and deal-review corpus and emits a machine readable "firm profile" — screening criteria, IC memo style guide, decision bias matrix — that drops directly into Zarna\'s Analyst and Memo agents as a system prompt + few shot bundle.'
...
HERO_TITLE = 'Zarna Adversarial Replay Board'
